    ...
    async function initializeCompany() {
      try {
        ...
        if(loginResult) {
          ...
          if(events) {
            const selected_event = events.filter((event) => event.event_id === EventId);
            if(selected_event) {
        ...
        <div className="py-8">
                ...
                <div className='px-2 py-2 font-semibold text-2xl bg-gray-200 border-none rounded-md shadow-md hover:shadow-none hover:opacity-90 hover:bg-purple-200 active:opacity-60 transition duration-300 cursor-pointer'>
                  {event.payment_status && 
                    (event.payment_status === 0 && <div>{"N/A"}</div> 
                        ...
                        || event.payment_status === 2 && <div>{"Paid 10%!"}</div> 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
            {submissions.map((submission) => (
              <div key={submission.submission_id} className='border rounded-md bg-gray-100 cursor-pointer' onClick={event.payment_status === 4 ? () => {router.push(`${event_id}/submission/${submission.submission_id}`)} : () => {}}>
