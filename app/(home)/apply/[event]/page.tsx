    ...
    async function func() {
      ...
      if (eventId && user && user instanceof UserInstance) {
        ...
        user.getSubmissions().then((submissions) => {
          if (
            submissions &&
            submissions.some((sub) => sub.event_id === event!.event_id)
          ) {
            router.push("/home");
          } else {
            setLoading(false);
    ...
    try {
      if (user instanceof UserInstance) {
        const res = await user.createSubmission({
          ...
          project_name: `${user.firstName} ${user.lastName}'s -> ${
            event!.event_name
          }`,
          project_description: `${user.firstName} ${user.lastName}'s -> ${
            event!.event_name
          } -> Description`,
    ...
    <div className="flex flex-col ml-10 mr-10 mb-10 mt-4">
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col bg-white rounded-lg p-5">
          ...
          <div className="font-poppins sm:text-sm text-xs mt-1 mb-4 text-logo-purple">
            Please contact {(sponsor ?? { email: "..." }).email} for any
            questions.
          </div>
          <div className="font-poppins sm:text-lg font-semibold text-sm mt-4 text-logo-purple">
            Prize Pool
